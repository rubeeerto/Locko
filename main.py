...
async def referral_program(message: types.Message):
    ...
    # Форматуємо текст з особистим посиланням користувача
    share_text = f"Привіт! Приєднуйся до нашого боту! 📱 Завдяки тобі ми зможемо зростати та робити для тебе ще більше 🚀\n\n{referral_link}"
    encoded_text = urllib.parse.quote(share_text)
    # Використовуємо параметр url з referral_link для передачі параметра start
    encoded_url = urllib.parse.quote(referral_link)
    # Використовуємо обидва параметри - посилання буде в превью та в тексті (клікабельне)
