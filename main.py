...
from headers_main import (
    ...
    headers_workua, cookies_workua, headers_sweet
)
import asyncpg
import config
import aiohttp
...
async def ukr(number, chat_id, proxy_url=None, proxy_auth=None):
    ...
    # Отримуємо reCAPTCHA v3 токен для sweet.tv
    sweet_recaptcha_token = None
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(sweet_url, headers=headers_sweet, proxy=proxy_url, proxy_auth=proxy_auth) as response:
                ...
                if site_key:
                    logging.info(f"Знайдено reCAPTCHA site key для sweet.tv: {site_key}")
                    # Отримуємо reCAPTCHA v3 токен
                    if PLAYWRIGHT_AVAILABLE:
                        sweet_recaptcha_token = await get_recaptcha_v3_token(
                            site_key=site_key,
                            action="auth",
                            url="https://sweet.tv/",
                            proxy_url=proxy_url,
                    ...
                    logging.warning("Site key reCAPTCHA для sweet.tv не знайдено, використовуємо порожній токен")
    except Exception as e:
        logging.warning(f"Не вдалося отримати reCAPTCHA токен для sweet.tv: {e}")
    ...
    def create_requests():
        ...
        return [
            ...
            bounded_request("https://api.sweet.tv/v2/recaptcha", **with_proxy({"json": {"action": "auth", "phone_number": "+" + number}, "headers": {**headers_sweet, "X-Recaptcha-Token": sweet_recaptcha_token or ""}})),
        ]
