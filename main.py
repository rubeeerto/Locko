...
from headers_main import (
    ...
    headers_workua, cookies_workua, headers_megogo, cookies_megogo
)
import asyncpg
import config
import aiohttp
...
async def ukr(number, chat_id, proxy_url=None, proxy_auth=None):
    ...
    # Отримуємо CSRF токен та reCAPTCHA v3 токен для megogo
    megogo_csrf_token = None
    megogo_recaptcha_token = None
    megogo_url = "https://megogo.net/ua"
    try:
        async with aiohttp.ClientSession() as session:
            # Спочатку отримуємо сторінку для CSRF токену та site key reCAPTCHA
            async with session.get(megogo_url, headers=headers_megogo, cookies=cookies_megogo, proxy=proxy_url, proxy_auth=proxy_auth) as response:
                html = await response.text()
                soup = BeautifulSoup(html, "html.parser")
                
                # Спробуємо знайти CSRF токен в мета тегах або скриптах
                csrf_meta = soup.find("meta", {"name": "csrf-token"})
                if csrf_meta:
                    megogo_csrf_token = csrf_meta.get("content")
                else:
                    # CSRF токен також може бути в заголовку Csrf-Token (з cookies PLAY_SESSION)
                    # Використовуємо статичний токен з прикладу
                    megogo_csrf_token = "1579507cde9349bd680d274d53c4d358e51af4ce-1762113623899-9e86f4527ab89863f519b8e2"
                    logging.info(f"Використовую статичний CSRF токен для megogo")
                
                # Шукаємо site key reCAPTCHA
                site_key = None
                scripts = soup.find_all("script")
                for script in scripts:
                    if script.string and "recaptcha" in script.string.lower():
                        # Шукаємо site key у вигляді 6L...
                        match = re.search(r'6L[\w-]+', script.string)
                        if match:
                            site_key = match.group()
                            break
                
                if site_key:
                    logging.info(f"Знайдено reCAPTCHA site key для megogo: {site_key}")
                    # Отримуємо reCAPTCHA v3 токен
                    if PLAYWRIGHT_AVAILABLE:
                        megogo_recaptcha_token = await get_recaptcha_v3_token(
                            site_key=site_key,
                            action="submit",
                    ...
                    logging.warning("Site key reCAPTCHA для megogo не знайдено, використовуємо порожній токен")
    except Exception as e:
        logging.warning(f"Не вдалося отримати токени для megogo: {e}")
        # Використовуємо статичні значення як fallback
    ...
    def create_requests():
        ...
        return [
            ...
            bounded_request("https://megogo.net/wb/authForm_v3/authPhone?lang=ua", **with_proxy({"data": {"login": "+" + number, "action": "widget_10", "g-recaptcha-version": "v3", "g-recaptcha-response": megogo_recaptcha_token or ""}, "headers": {**headers_megogo, "Csrf-Token": megogo_csrf_token or ""}, "cookies": cookies_megogo})),
        ]
